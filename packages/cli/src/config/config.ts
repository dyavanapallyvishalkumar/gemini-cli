...
export async function parseArguments(
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('zolt')
    .usage(
      'Usage: zolt [options] [command]\n\nZOLT CLI - Defaults to interactive mode. Use -p/--prompt for non-interactive (headless) mode.',
    )
    .option('debug', {
      alias: 'd',
      type: 'boolean',
    ...
    .command('$0 [query..]', 'Launch ZOLT CLI', (yargsInstance) =>
      yargsInstance
        .positional('query', {
          description:
