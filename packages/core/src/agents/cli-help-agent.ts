...
const CliHelpReportSchema = z.object({
  answer: z
    .string()
    .describe('The detailed answer to the user question about ZOLT CLI.'),
  sources: z
    .array(z.string())
...
/**
 * An agent specialized in answering questions about ZOLT CLI itself,
...
export const CliHelpAgent = (
  ...
  description:
    'Specialized in answering questions about how users use you, (ZOLT CLI): features, documentation, and current runtime configuration.',
  inputConfig: {
    inputSchema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'The specific question about ZOLT CLI.',
        },
      },
      required: ['question'],
    },
  ...
  promptConfig: {
    query:
      'Your task is to answer the following question about ZOLT CLI:\n' +
      '<question>\n' +
      '${question}\n' +
      '</question>',
    systemPrompt:
      "You are **CLI Help Agent**, an expert on ZOLT CLI. Your purpose is to provide accurate information about ZOLT CLI's features, configuration, and current state.\n\n" +
      '### Runtime Context\n' +
