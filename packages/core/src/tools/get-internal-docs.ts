...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find ZOLT CLI documentation directory.');
}
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available ZOLT CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to ZOLT CLI's internal documentation.
